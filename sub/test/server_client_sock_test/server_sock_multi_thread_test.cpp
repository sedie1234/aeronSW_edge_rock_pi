...
#define SERVER_PORT 6236

// Client 정보를 담는 구조체 (int socket, sockaddr_in client_Address)
struct Client_Info{
    int client_socket;
...
vector<Client_Info*> client_Conn_Pool; 
vector<pthread_t> client_management_Threads;
...
int main(){
    ...
    //바인딩 실패시
    if(bind_result < 0){
        cerr << "failed Binding " << bind_result <<endl;
        perror("bind");
        close(server_socket);
        return -1;
    }
    ...
    //서버 클라이언트 요청 대기...
    listen(server_socket, 10); // 백로그 크기 : 10(동시에 처리 가능한 연결 요청 최대 개수)
    ...
    while(true){
        ...
        if((string)msg == "li" or (string)msg=="list"){
            ...
            for(Client_Info* c : client_Conn_Pool){
                cout << inet_ntoa(c->client_Addr.sin_addr) << ":" << ntohs(c->client_Addr.sin_port) <<endl;
            }

        }else if((string)msg == "exit"){
            cout << "terminate server" << endl;
            pthread_detach(thread_listen); // TCB 제어를 위한 detach. 
            break;
    ...
    for (Client_Info* c : client_Conn_Pool){ // for c in client_Conn_Pool
        cout << "Deallocate Client Pool memory : " << c << endl;
        delete c;
    }
    // Thread 메모리 할당 해제
    for (pthread_t hThread : client_management_Threads){
...
void* client_accept(void* data){
    ...
    while(1){
        ...
        client_Conn_Pool.push_back(client_Data);
        ...
        client_management_Threads.push_back(thread_client);
    }
    return nullptr;
}
...
void* handle_client(void* data){
    ...
    sockaddr_in client_Addr = client_Data->client_Addr;

    char buffer[PACKET_SIZE]; //PACKET_SIZE 1024
    int recv_size;

    do{
        memset(buffer, 0, PACKET_SIZE); //TODO : 패킷사이즈를 넘어가는 데이터의 경우 다 들어올때까지 기다려야함. 
        ...
        if(recv_size > 0){
            ...
            send(client_socket, buffer, recv_size, 0); // 테스트용. 서버에서 받은 데이터 그대로 전송 (echo 서버)
        }

    }while(recv_size > 0); 
    
    cout << "Timeout or disconnected client" << endl; 

    // 전송이 끝나면 클라이언트 Pool 에서 삭제 하고, 소켓도 해제
    for (size_t i=0; i < client_Conn_Pool.size(); ++i){
        if(client_Conn_Pool[i]->client_socket == client_socket){
            // 클라이언트 정보 삭제
            cout << "disconnect Client : " << inet_ntoa(client_Conn_Pool[i]->client_Addr.sin_addr) \
            << " : " << ntohs((client_Conn_Pool[i]->client_Addr.sin_port)) << endl;
            send(client_socket, buffer, recv_size, 0);
            client_Conn_Pool.erase(client_Conn_Pool.begin() + i);
        }
    }
    delete client_Data;
    close(client_socket);
