...
void* big_send_message(void* server_data);
...
int main(){
    ...
    while(true){
        ...
        if((string)scf == "exit"){
            ...
            pthread_detach(th_handler); // Thread 분리. 프로그램 종료시 OS 에서 제거
            break;
        }else{
            //TODO 
        }
    ...
    //종료 코드
    pthread_join(th_handler, nullptr); // 메인 쓰레드 종료까지 대기
    // 연결된 소켓, 메모리 할당 해제
    ...
    // 나머지 Thread 분리
    for (pthread_t rt : read_thread_Manager){
        cout << "deallocate read_thread" << rt << endl;
        pthread_detach(rt);
    }
    for (pthread_t wt : write_thread_Manager){
        cout << "deallocate write_thread" << wt << endl;
        pthread_detach(wt);
    }
...
void* gen_verbose_threads(vector<Server_Info*>* conn_pool_ptr, vector<pthread_t>* read_thread_Manager, vector<pthread_t>* write_thread_Manager, size_t thr_count){
    ...
    for (Server_Info* server_Data : *conn_pool_ptr){
        ...
        //임시, big message send thread 하나 생성 (소켓당 하나)
        pthread_t* b_w_Thread = new pthread_t;
        if (pthread_create(b_w_Thread, nullptr, big_send_message, (void*)server_Data)){
            cerr << "error creating write thread" << endl;
            perror("w_thread err");
            delete b_w_Thread;

        }else{
            write_thread_Manager->push_back(*b_w_Thread);
            delete b_w_Thread;
        }
