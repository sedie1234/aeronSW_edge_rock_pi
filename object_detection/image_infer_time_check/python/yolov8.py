...
if __name__ == '__main__':
    ...
    program_elapsed = (program_end_time - program_start_time)  # 초 단위를 마이크로초로 변환
    npu_elapsed = (npu_end_time - npu_start_time)
    real_elapsed = (real_end_time - real_start_time)
    post_process_time = (real_end_time - npu_end_time)
    print(f"Inference elapsed time: {npu_elapsed:.4f} s")
    print(f"Program elapsed time: {program_elapsed:.4f} s")
    print(f"Real elapsed time: {real_elapsed:.4f} s")
    print(f"Post Process time : {post_process_time:.4f}")
